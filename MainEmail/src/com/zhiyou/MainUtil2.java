...
public class MainUtil2 {
	...
	public static void setMain(String main, String name) {
		...
		String myEmailAccount = "15638400969@163.com";// 邮箱账号
		String myEmailPassword = "zhuang0905";
	...
	public static MimeMessage createMimeMessage(Session session, String sendMail, String receiveMail, String name) {
		...
		try {
			message.setFrom(new InternetAddress("15638400969@163.com", "安防科技老总", "UTF-8"));
			// 3. To: 收件人（可以增加多个收件人、抄送、密送）
			message.setRecipient(MimeMessage.RecipientType.TO, new InternetAddress(receiveMail, "安防科技会员", "UTF-8"));

			// 4. Subject: 邮件主题（标题有广告嫌疑，避免被邮件服务器误认为是滥发广告以至返回失败，请修改标题）
			message.setSubject("安防科技老总给你发的邮件", "UTF-8");
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		...
		String content = "您于" + ft.format(date) + "注册安防科技，您的验证码为" + name;
		try {
